# Articles 索引

本目录存放关于 AI 使用和本仓库相关的感想、思考、感悟等文章。

---

## 文章列表

| 序号 | 标题 | 文件 | 状态 | 简介 |
|------|------|------|------|------|
| 1 | AI Playbook 仓库初始化：从空目录到完整技能体系 | [`ai-playbook-init.md`](ai-playbook-init.md) | ✅ 已完成 | 一次对话从零搭建起 ai-playbook 仓库：四技能体系、双语文档、GitHub Pages，含关键决策与产出清单 |
| 2 | 从三次审计到一套流水线：AI Playbook 资产创建体系的建立 | [`asset-creation-pipeline.md`](asset-creation-pipeline.md) | ✅ 已完成 | 审计三个技能发现同一个 D4 缺陷，由此催生了 rule 文件、skill-creator 技能、以及分类-创建-审计的完整流水线 |
| 3 | AI Playbook 资产分类梳理与 Pi 双向同步 | [`ai-playbook-pi-sync-setup.md`](ai-playbook-pi-sync-setup.md) | ✅ 已完成 | 厘清八种资产类型的区别，用软链接打通仓库与 Pi 的双向同步，并创建了首个 Role 资产 |
| 4 | AI Playbook 全站 Markdown HTML 化：批量转化与导航链接体系搭建 | [`ai-playbook-md-to-html-batch.md`](ai-playbook-md-to-html-batch.md) | ✅ 已完成 | 将仓库中 6 个 Markdown 文件全部转为极简科技感 HTML，构建完整导航链接体系，并修复了 macOS 大小写冲突问题 |
| 5 | pi Extensions 从科普到实战：打造 article-sync 自动文章转化扩展 | [`pi-extension-article-sync.md`](pi-extension-article-sync.md) | ✅ 已完成 | 从 Extensions 概念科普到实战实现 article-sync 扩展，自动监控 articles/ 目录并触发 md-to-html 转化 |
| 6 | 为 AI Playbook 搭建私信留言价值过滤器 | [`message-filter-skill-setup.md`](message-filter-skill-setup.md) | ✅ 已完成 | 从粉丝互动出发，将留言过滤思路沉淀为技能：六维评估、三层过滤链、审计修复全记录 |

---

> 待后续补充。文章可手动撰写或通过 AI 辅助记录。新增文章时请在本文件中添加对应条目。

## 最新文章

| 序号 | 标题 | 文件 | 状态 | 简介 |
|------|------|------|------|------|
| 6 | 为 AI Playbook 搭建私信留言价值过滤器 | [`message-filter-skill-setup.md`](message-filter-skill-setup.md) | ✅ 已完成 | 从粉丝互动出发，将留言过滤思路沉淀为技能：六维评估、三层过滤链、审计修复全记录 |
