...
| 8 | **Cognitive Drilling** | [`cognitive-drilling/SKILL.md`](cognitive-drilling/SKILL.md) | ✅ 可用 | 认知钻井机——将任何一句话、一个现象、一个问题向下挖掘六层：连续追问 → 规律提取 → 反向思维 → 多学科拆解 → 研究级问题 → 机会发现，输出完整结构化分析报告和 TOP 20 核心问题树。 | ✅ 已安装 |
| 9 | **Prompts Audit** | [`prompts-audit/SKILL.md`](prompts-audit/SKILL.md) | ✅ 可用 | 对提示词（Prompt）文件进行结构化质量审计，按 P1-P6 六个维度（目标清晰度/变量设计/输出格式/防护边界/示例完整性/平台与版本）逐条评分，输出通过/警告/缺失报告和可操作的修复建议。 | ⏳ 待安装 |
...
| 2026-06-15 | 新增技能：Cognitive Drilling（认知钻井机，六层深度挖掘框架，结构化分析报告） |
| 2026-06-15 | 新增技能：Prompts Audit（提示词质量审计，P1-P6 六维度评分，支持极简Prompt/系统提示词/多平台对比） |
